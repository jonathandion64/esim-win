...
{p_html}
  ...
  <script>
    ...
    document.getElementById('buy-now-btn').addEventListener('click', function() {{
      ...
      // Cross-domain tracking: pass GA4 client_id (_ga) + gclid so simoptions.com
      // can attribute the purchase back to the original Google Ads click.
      // GA4 linker cannot auto-decorate window.open/programmatic clicks, so we do it manually.
      var url = selectedUrl;
      // Extract gclid from _gcl_aw cookie (set automatically by Google Ads auto-tagging)
      ...
      function openUrl(clientId) {{
        ...
        window.open(params.length ? url + sep + params.join('&') : url, '_blank', 'noopener');
      }}
      if (typeof gtag === 'function') {{
        gtag('get', 'G-FWN5WTYV2V', 'client_id', openUrl);
      }} else {{
        openUrl('');
      }}
    }});
